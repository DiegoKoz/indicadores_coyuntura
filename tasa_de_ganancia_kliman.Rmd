...
```{r message=FALSE, warning=FALSE}
fixed_assets_hc <- read_csv("fuentes/Table 6.3. Historical-Cost Net Stock of Private Fixed Assets by Industry Group and Legal Form of Organization.csv", skip = 4) %>%
  select(-Line) %>%
  rename(sector = X2) %>%
  na.omit(fixed_assets_hc) %>%
  gather(.,
           key = fecha,
         value = stock,
         2:95) %>%
...
fixed_assets_cc  <- read_csv("fuentes/stock_current_cost.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
  na.omit(fixed_assets_hc) %>%
...
rop <- fixed_assets_hc %>%
  ...
  left_join(fixed_assets_cc, by = c("sector.x", "fecha")) %>% 
