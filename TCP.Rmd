...
employ_us <- base_employ_us %>% 
  mutate(mes  = str_remove(base_employ_us$Period, "M"),
         fecha = parse_date_time(paste(Year, mes, sep = ""), orders = "ym"),
         fecha = ymd(fecha)) %>% 
...
employ_us <- employ_us %>% 
  mutate(i_empleo_us = empleo_us / aniob_emp_us$empleo_us * 100) %>% 
  select(-empleo_us)
employ_us  
...
manuf_us <-  base_manuf_us %>% 
  mutate(fecha = DATE,
         ...
         fecha1 = parse_date_time(paste(anio, mes, sep = ""), orders = "ym" )) %>%
...
manuf_us <-  manuf_us %>% 
  mutate(i_manuf_us = indice_manuf/aniob_manuf_us$indice_manuf * 100) %>% 
  select(-indice_manuf)
manuf_us
...
cpi_anual <- file <- read_csv("C:/Archivos/Datos/BLS/file.csv") %>%
  group_by(Year) %>% 
...
manuf_arg <- base_manuf_arg04 %>%
  ...
  mutate(i_manuf_arg = industria_manufacturera_total/aniob_va04$industria_manufacturera_total) %>% 
...
empleo_arg <-  empleo_nacional_serie_trimestral %>% 
  mutate(i_empleo_arg = empleo_arg/aniob_emp_ar$empleo_arg) %>% 
  select(-empleo_arg)
empleo_arg
```
