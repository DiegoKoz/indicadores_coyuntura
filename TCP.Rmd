...
# cpi_anual <- read_csv("C:/Archivos/Datos/BLS/file.csv") %>%
#   group_by(Year) %>% 
#   summarise(cpi = mean(Value))
# 
# cpi_anual %>% filter(Year >= 1982, Year <= 1984) %>% 
#   mutate( cpi = mean(cpi))
...
manuf_arg <- base_manuf_arg04 %>%
  ...
  mutate(i_manuf_arg = generar_indice(serie = industria_manufacturera_total,
                                    fecha = fecha,
                                    fecha_base = "2017-01-01")) %>% 
...
productividad_arg <- productividad_arg %>% 
  complete(fecha = seq.Date(min(fecha), max(fecha), by="month")) %>% 
  mutate(i_productividad_arg = na_interpolation(i_productividad_arg,option = 'linear'))
productividad_arg
...
```{r}
#promedios 59-72
promedios_59_72 <- read_csv("fuentes/promedios_59_72.txt") %>% spread(variable,valor)
...
tcp <- tc_mensual %>% 
  ...
  mutate(ipc_arg_b = i_ipc9/promedios_59_72$ipc_prom,
         ...
         tcp_arbitrario = promedios_59_72$tcc_prom * ((ipt_arg_b / ipt_us_b) * (ipc_us_b / ipc_arg_b)),
         sobrevaluacion = tcp_arbitrario / promedios_59_72$tcp_arb_tcc_prom,
