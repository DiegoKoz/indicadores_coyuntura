...
```{r message=FALSE, warning=FALSE}
stock_corp_hc <- read_csv("fuentes/Table 6.3. Historical-Cost Net Stock of Private Fixed Assets by Industry Group and Legal Form of Organization.csv", skip = 4) %>%
  select(-Line) %>%
  rename(sector = X2) %>%
...
```{r}
stock_corp_cc  <- read_csv("fuentes/stock_current_cost.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
  na.omit(stock_corp_hc) %>%
...
```{r}
stock_total_cc <- read_csv("fuentes/stock_total_cc.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
...
```{r}
net_va <- read_csv("fuentes/Net Value Added by Sector.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
  na.omit(net_va) %>%
...
persons_engaged_29_48 <- read_csv("fuentes/Persons Engaged in Production by Industry_29_48.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
...
persons_engaged_48_87 <- read_csv("fuentes/Persons Engaged in Production by Industry_48_87.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
...
persons_engaged_87_00 <- read_csv("fuentes/Persons Engaged in Production by Industry_87_00.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
...
persons_engaged_98_18 <- read_csv("fuentes/Persons Engaged in Production by Industry_98_18.csv", 
    skip = 4) %>% 
  select(-Line) %>%
  rename(sector = X2) %>%
